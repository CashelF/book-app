...
class RecommendationsRepository:
    def get_user_context(user_id):
        ...
        return {
            'id': user.id,
            'name': user.name,
            'preferences': user.preferences  # Assuming preferences is a precomputed embedding
        }

    def get_all_parameters():
        books = Book.query.all()
        return [{'id': book.id, 'title': book.title, 'parameters': book.parameters.parameters} for book in books]
